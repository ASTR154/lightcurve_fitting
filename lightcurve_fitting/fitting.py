...
def lightcurve_mcmc(lc,model,priors=None,p_min=None,p_max=None,p_lo=None,p_up=None,nwalkers=100,nsteps=1000,nsteps_burnin=1000,
model_kwargs=None,show=False,save_plot_as='',save_sampler_as='',use_sigma=False,sigma_type='relative'):
...
#added gap
    def log_posterior(p):
#separated the function name from the function
        log_prior=0.
...
#added gap
    ...
    if show or save_plot_as:
        ...
        if show:
            plt.show()
#separated what is returned
    return sampler

def lightcurve_corner(lc,model,sampler_flatchain,model_kwargs=None,num_models_to_plot=100,lcaxis_posn=(0.7,0.55,0.2,0.4),
...
xscale='linear',filters_to_model=None,label_filters=True,lc_plot_kwargs=None,model_plot_kwargs=None):
#separeted function name from the function and separated parameters
    if model_kwargs!=None:
        raise Exception(model_kwargs_warning)
...
#separated a funtion from variables
...
#added gap
    if use_sigma and model.input_names[-1]!='\\sigma':
        model.input_names.append('\\sigma')
        model.units.append(u.dimensionless_unscaled)
#separated funtions from variable
