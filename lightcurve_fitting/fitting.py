# general changes:
# -- added lines between separate logical blocks and loops (if/else, while, etc.)
# -- added spaces between function arguments
# -- changed all strings with double backslashes to raw strings with single backslashes (should not break LaTeX)
...
import astropy.units as apu  #this change makes it more clear that we are referring to an astropy submodule
import emcee, corner  #added spacing in lines 4 - 6
...
#added line to separate body from import statements
PRIOR_WARNING = 'The p_max/p_min keywords are deprecated. Use the priors keyword instead.'  #capitalized these lines to signify that they are constants
MODEL_KWARGS_WARNING = 'The model_kwargs keyword is deprecated. These are now included in the model intialization.'

def lightcurve_mcmc(lc, model, priors=None,  #separated parameters into multiple lines based on how I perceive them to relate to each other
                    p_min=None, p_max=None,
                    p_lo=None, p_up=None,
                    nwalkers=100, nsteps=1000, nsteps_burnin=1000,
                    model_kwargs=None, show=False,
                    save_plot_as='', save_sampler_as='',
                    use_sigma=False,sigma_type='relative'):
    
    #added lines to separate logical blocks
    if model_kwargs != None: #added spaces in binary operator
        raise Exception(MODEL_KWARGS_WARNING)
    
    if model.output_quantity == 'flux':  #added spaces in binary operator
        lc.calcFlux()
    elif model.output_quantity == 'lum':
        lc.calcAbsMag()
        lc.calcLum()
    
    if use_sigma and model.input_names[-1] != r'\sigma':
        model.input_names.append(r'\sigma')  #changed to raw strings to eliminate escape characters
        model.units.append(apu.dimensionless_unscaled)
    ndim = model.nparams
    ...
    for param,prior,p0,p1 in zip(model.input_names, priors, p_lo, p_up):  #added spaces between parameters
        if p0<prior.p_min:
    ...
    if show or save_plot_as:
        fig, ax = plt.subplots(ndim,2,figsize=(12.0, 2.0*ndim))  #added spaces in binary operator and after comma
        ax1 = ax[:,0]
...
def lightcurve_corner(lc, model, sampler_flatchain, model_kwargs=None, #separated parameters based on their association to each other
                      num_models_to_plot=100, lcaxis_posn=(0.7,0.55,0.2,0.4),
                      filter_spacing=1.0,
                      tmin=None, tmax=None, t0_offset=None,
                      save_plot_as='',ycol=None, textsize='medium', param_textsize='large',
                      use_sigma=False, xscale='linear',
                      filters_to_model=None, label_filters=True,
    ...
    if ycol == 'lum':
        dycol = 'dlum'
        yscale = 10.0**np.round(np.log10(y_fit.max()))
        ylabel=r'Luminosity $L_\nu$ (10$^{{{:.0f}}}$ erg s$^{{-1}}$ Hz$^{{-1}}$)+Offset'.format(np.log10(yscale) + 7)#W-->erg/s
    elif ycol == 'absmag':
        dycol = 'dmag'
        yscale = 1.
    ...
    if model_plot_kwargs == None:
        ...
        #in next line added 2 spaces before comment as recommended by PEP8
    ...
    sigfigs = 1  #separated into multiple lines
    percentiles = (15.87, 50.0, 84.14)
    axis=0
    ...
    for quant,unc,dec in zip(quantiles, uncertainties, roundto):
        ...
        if dec < 0:
            dec = 0
        if upper == lower:
            texstring = r'{{:.{0:d}f}} \pm {{:.{0:d}f}}'.format(dec).format(center,upper)
        else:
    ...
    if model.input_names == None or model.units == None: 
        ...
        for var,value,unit in zip(model.input_names, texstrings,model.units):
            if isinstance(unit,apu.quantity.Quantity):
                value = r'({}) \times 10^{{{:.1f}}}'.format(value, np.log10(unit.value)).replace('.0}', '}')
                unit = unit.unit
