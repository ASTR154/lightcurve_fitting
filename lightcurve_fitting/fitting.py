...
#2 lines around function
def lightcurve_mcmc(lc, model, priors=None, p_min=None, p_max=None, p_lo=None, p_up=None,
        nwalkers= 100, nsteps= 1000, nsteps_burnin= 1000, model_kwargs=None, show=False, save_plot_as='',
        save_sampler_as='', use_sigma=False, sigma_type='relative'): # added spacing after comma + spacing after assignments to real values
    if model_kwargs!=None:
        raise Exception(model_kwargs_warning)
    if model.output_quantity == 'flux':  #Spaces for readability around ==
        lc.calcFlux()
    elif model.output_quantity == 'lum':#Spaces for readability around ==
        lc.calcAbsMag()
        lc.calcLum()
    ...
    ndim=model.nparams
    #DEPRECATED
    if p_min == None: #Spaces for readability around ==
        p_min=np.tile(-np.inf,ndim)
    elif len(p_min) == ndim: #Spaces for readability around ==
    ...
    elif len(p_max) == ndim: #Spaces for readability around ==
    ...
    if p_lo==None:
        p_lo=p_min
    elif len(p_lo) == ndim: #Spaces for readability around ==
        p_lo=np.array(p_lo,float)
    else:
        raise Exception('p_lo must have length {:d}'.format(ndim))
    if len(p_up) == ndim: #Spaces for readability around ==
    ...
    elif len(priors) != ndim: #Spaces for readability around !=
    ...
    def log_posterior(p):
        log_prior = 0. #Spaces for readability around =
        for prior, p_i in zip(priors, p): #Spaces for readability around ,
            log_prior += prior(p_i)  #Spaces for readability around +=
        if np.isinf(log_prior):
            return log_prior
        log_likelihood=model.log_likelihood(lc, p, use_sigma=use_sigma, sigma_type=sigma_type) #Spaces for readability around ,
        return log_prior + log_likelihood #Spaces for readability 
    sampler = emcee.EnsembleSampler(nwalkers,ndim,log_posterior) #Spaces for readability 
    starting_guesses = np.random.rand(nwalkers,ndim)*(p_up-p_lo)+p_lo #Spaces for readability 
...
#2 lines around functions
def lightcurve_corner(lc, model, sampler_flatchain, model_kwargs=None, num_models_to_plot=100,
                      lcaxis_posn=(0.7,0.55,0.2,0.4), filter_spacing=1., tmin=None, tmax=None, t0_offset=None, save_plot_as='',
                      ycol=None, textsize='medium', param_textsize='large', use_sigma=False, xscale='linear', filters_to_model=None,
                      label_filters=True, lc_plot_kwargs=None, model_plot_kwargs=None): # #Spaces for readability + made it go over multiple lines
    if model_kwargs!=None:
        raise Exception(model_kwargs_warning)
    if ycol==None:
    ...
    for var in ['t_0','t_\\mathrm{max}']:
        if var in model.input_names:
            i_t0 = model.input_names.index(var) #Spaces for readability 
            if t0_offset==None:
                t0_offset=np.floor(sampler_flatchain_corner[:,i_t0].min())
            if t0_offset != 0.: #Spaces for readability 
    ...
    if xscale=='log':
        ...
        lc=lc.copy()
    lc['MJD'] -= t0_offset #Spaces for readability 
    lc[ycol] /= yscale #Spaces for readability 
    lc[dycol] /= yscale #Spaces for readability 
    lc.plot(xcol='MJD', ycol=ycol, offset_factor=filter_spacing, appmag_axis=False, tight_layout=False, **lc_plot_kwargs) #Spaces for readability 
    plt.autoscale(False)
    ...
    quantiles = np.percentile(sampler_flatchain, percentiles, axis=axis).T #Spaces for readability 
    uncertainties=np.diff(quantiles)
    smaller_unc=np.amin(uncertainties, axis=-1) #Spaces for readability 
    ...
    if model.input_names==None or model.units==None:
        ...
        paramtexts=[]
        for var,value,unit in zip(model.input_names, texstrings, model.units): #Spaces for readability 
