...
# Define the lightcurve function  # Added some comments 
def lightcurve_mcmc(lc,model,priors=None,
                    p_min=None,p_max=None,
                    p_lo=None,p_up=None,
                    nwalkers=100,nsteps=1000,
                    nsteps_burnin=1000,model_kwargs=None,
                    show=False,save_plot_as='',
                    save_sampler_as='',use_sigma=False,
                    sigma_type='relative'):

    if model_kwargs is not None:
        raise Exception(model_kwargs_warning)

    # Compute quantities based on model output
    ...
    #DEPRECATED
    if p_min is None:   # Changed != into is None
        p_min = np.tile(-np.inf, ndim)  # Made a space after "," to make it easier to read 
    elif len(p_min) == ndim:
    ...
    # Check the prior limits
    for param, prior, p0, p1 in zip(model.input_names,priors,p_lo,p_up):
        if p0 < prior.p_min:
            raise Exception(
                f'starting guess for {param} (p_lo={p0}) is outside prior (p_min={prior.p_min})'
                )
        if p1 > prior.p_max:
            raise Exception(
                f'starting guess for {param} (p_up={p1}) is outside prior (p_max={prior.p_max})'
                )
    ...
    def log_posterior(p):
        ...
        if np.isinf(log_prior):
            return log_prior
        log_likelihood = model.log_likelihood(lc, p,
                                              use_sigma=use_sigma,
    ...
    pos,_,_ = sampler.run_mcmc(starting_guesses,
                               nsteps_burnin,
                               progress=True,
    ...
    if save_sampler_as:
        ...
        print(f'saving sampler.flatchain as {save_sampler_as}')
    ...
    if show or save_plot_as:
        ...
        if save_plot_as:
            print(f'saving chain plot as {save_plot_as}')
...
# Define the lightcurve-corner function
def lightcurve_corner(lc, model,
                      sampler_flatchain,
                      model_kwargs=None,
                      ...
                      save_plot_as='', ycol=None,
                      textsize='medium', param_textsize='large',
                      use_sigma=False, xscale='linear',
                      filters_to_model=None, label_filters=True,
                      lc_plot_kwargs=None, model_plot_kwargs=None):  # Line break to make one line shorter
    
    # Raise an error if model_kwargs is used
    if model_kwargs is not None:
        raise Exception(model_kwargs_warning)
    ...
    fig = corner.corner(sampler_flatchain_corner,
                        ...
                        label_kwargs = {'size':textsize})

    corner_axes = np.array(fig.get_axes()).reshape(sampler_flatchain.shape[-1],
                                                   sampler_flatchain.shape[-1])
    ...
    if lc_plot_kwargs is None:
        lc_plot_kwargs = {}

    if model_plot_kwargs is None:
        model_plot_kwargs = {}
    ...
    for yfit, yfit1, filt, txt in zip(y_fit, y_fit1, ufilts, labels):
        ...
        if yfit1 is not None:
            ax.plot(xfit-t0_offset,
                    np.median(yfit1,axis=1)/yscale+offset,
                    color=filt.linecolor,
                    **model_plot_kwargs1)
    ...
    if model.input_names is None or model.units is None:
